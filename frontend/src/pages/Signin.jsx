...
const Signin = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post('http://localhost:3000/api/users/signin', formData);
            if (res.status === 200) {
                const { user, token, passwordExpiry } = res.data;
                sessionStorage.setItem("authToken", token);
                sessionStorage.setItem("authUser", JSON.stringify(user));
                sessionStorage.setItem("passwordExpiry", passwordExpiry);
                ...
                <div className="container px-6 py-24 mx-auto lg:py-32">
                    <div className="lg:flex">
                        <div className="lg:w-1/2">
                            <h1 className="mt-4 text-gray-600 font-medium dark:text-gray-300 md:text-lg">Welcome Back!</h1>
